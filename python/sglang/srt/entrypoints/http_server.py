...
@app.post("/v1/sessions/prune", dependencies=[Depends(validate_json_request)])
async def v1_prune_session(request: PruneSessionRequest, raw_request: Request):
    """Prune a session's KV cache without generation.

    This endpoint is used for out-of-band summary scenarios where the summary
    is generated by a different model. It performs prefix matching on the radix
    tree and prunes the cache without generating any tokens.
    ...
    """
    result = await _global_state.tokenizer_manager.prune_session(
